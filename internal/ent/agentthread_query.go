...
type AgentThreadQuery struct {
	...
	loadTotal  []func(context.Context, []*AgentThread) error
...
func (_q *AgentThreadQuery) QueryAgent() *AgentQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(agentthread.Table, agentthread.FieldID, selector),
...
func (_q *AgentThreadQuery) QueryThread() *ThreadQuery {
...
func (_q *AgentThreadQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *AgentThreadQuery) FirstIDX(ctx context.Context) int {
...
func (_q *AgentThreadQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *AgentThreadQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *AgentThreadQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *AgentThreadQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *AgentThreadQuery) WithAgent(opts ...func(*AgentQuery)) *AgentThreadQuery {
	query := (&AgentClient{config: _q.config}).Query()
...
func (_q *AgentThreadQuery) WithThread(opts ...func(*ThreadQuery)) *AgentThreadQuery {
	query := (&ThreadClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(agentthread.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.AgentThread.Query().
//		Select(agentthread.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *AgentThreadQuery) prepareQuery(ctx context.Context) error {
	...
	if agentthread.Policy == nil {
		return errors.New("ent: uninitialized agentthread.Policy (forgotten import ent/runtime?)")
	}
	if err := agentthread.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *AgentThreadQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AgentThread, error) {
	...
	if query := _q.withAgent; query != nil {
		if err := _q.loadAgent(ctx, query, nodes, nil,
			func(n *AgentThread, e *Agent) { n.Edges.Agent = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withThread; query != nil {
		if err := _q.loadThread(ctx, query, nodes, nil,
			func(n *AgentThread, e *Thread) { n.Edges.Thread = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AgentThreadQuery) loadAgent(ctx context.Context, query *AgentQuery, nodes []*AgentThread, init func(*AgentThread), assign func(*AgentThread, *Agent)) error {
...
func (_q *AgentThreadQuery) loadThread(ctx context.Context, query *ThreadQuery, nodes []*AgentThread, init func(*AgentThread), assign func(*AgentThread, *Thread)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AgentThread)
	for i := range nodes {
		fk := nodes[i].ThreadID
...
func (_q *AgentThreadQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(agentthread.Table, agentthread.Columns, sqlgraph.NewFieldSpec(agentthread.FieldID, field.TypeInt))
	_spec.From = _q.sql
...
func (_q *AgentThreadQuery) Modify(modifiers ...func(s *sql.Selector)) *AgentThreadSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *AgentThreadSelect) Modify(modifiers ...func(s *sql.Selector)) *AgentThreadSelect {
