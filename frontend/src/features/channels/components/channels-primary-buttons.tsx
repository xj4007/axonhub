    ...
    <div className='flex gap-2 overflow-x-auto md:overflow-x-visible'>
      ...
      <PermissionGuard requiredScope='write_channels'>
        <>
          ...
          {/* Add Search Channel - requires write_channels permission */}
          <Button
            variant='outline'
            className='shrink-0 space-x-1'
            onClick={() => setOpen('addSearch')}
            data-testid='add-search-channel-button'
          >
            <span>{t('channels.addSearchChannel', 'Add Search Channel')}</span> <IconPlus size={18} />
          </Button>
        </>
      </PermissionGuard>
    </div>
