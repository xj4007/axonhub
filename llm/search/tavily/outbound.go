...
func (t *OutboundTransformer) APIFormat() llm.APIFormat {
	return llm.APIFormatTavilySearch
}
...
func (t *OutboundTransformer) TransformRequest(ctx context.Context, llmReq *llm.Request) (*httpclient.Request, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal tavily search request: %w", err)
	}
...
func (t *OutboundTransformer) TransformResponse(ctx context.Context, httpResp *httpclient.Response) (*llm.Response, error) {
	...
	if err := json.Unmarshal(httpResp.Body, &resp); err != nil {
		return nil, fmt.Errorf("%w: failed to decode tavily response: %w", transformer.ErrInvalidResponse, err)
	}
	...
	if llmResp.Search.Query == "" {
		llmResp.Search.Query = queryStr
	}
...
func (t *OutboundTransformer) TransformStream(ctx context.Context, stream streams.Stream[*httpclient.StreamEvent]) (streams.Stream[*llm.Response], error) {
	return nil, fmt.Errorf("tavily search does not support streaming")
}
...
func (t *OutboundTransformer) AggregateStreamChunks(ctx context.Context, chunks []*httpclient.StreamEvent) ([]byte, llm.ResponseMeta, error) {
	return nil, llm.ResponseMeta{}, fmt.Errorf("tavily search does not support streaming")
}
...
func (t *OutboundTransformer) TransformError(ctx context.Context, httpErr *httpclient.Error) *llm.ResponseError {
	...
	// Tavily uses OpenAI-like error envelope in some cases; fall back to generic.
	var upstream struct {
